...
const handleFileSelected = (file: File | File[]): void => {
    ...
    if (!Array.isArray(file)) {
        ...
        console.log('Image sélectionnée dans ServiceEdit:', file.name);
    }
};
...
const submitForm = async (): Promise<void> => {
    ...
    try {
        ...
        // Étape 1: Upload de l'image si une nouvelle est sélectionnée
        let imageUrl = formData.value.image; // Image existante par défaut
        ...
        // Étape 2: Mise à jour du service avec les données JSON standard
        const serviceData = {
            ...
            image: imageUrl // Utiliser l'URL de l'image (soit nouvelle, soit existante)
        };
        
        // Appel à l'API pour mettre à jour le service avec des données JSON
    ...
    <div class="fixed inset-0 bg-primary bg-opacity-50 flex items-center justify-center z-50 overflow-auto">
        ...
        <div class="bg-secondary rounded-lg shadow-lg w-full max-w-lg mx-4 my-8">
            ...
            <div class="px-6 py-4 border-b border-emphasis">
                <h3 class="text-lg font-semibold text-primary">Modifier le service</h3>
            </div>
            ...
            <div class="px-6 py-4 max-h-[70vh] overflow-y-auto">
                <form @submit.prevent="submitForm">
                    ...
                    <!--Nom du service-->
                    <div class="mb-4">
                        <label for="name" class="block text-sm font-medium text-primary-ghost mb-1">Nom du
                            service</label>
                    ...
                    <!--Type de service-->
                    <div class="mb-4">
                        <label for="type" class="block text-sm font-medium text-primary-ghost mb-1">Type de
                            service</label>
                    ...
                    <div class="mb-4">
                        ...
                        <!-- Utilisez @file-selected pour capturer l'événement -->
                        <ImageUploadButton :initial-image="formData.image" @file-selected="handleFileSelected" />
                        <p v-if="selectedImage" class="mt-2 text-sm text-green-600">Nouvelle image sélectionnée: {{
                            selectedImage.name }}</p>
                        <p v-else-if="formData.image" class="mt-2 text-sm text-blue-600">Image actuelle: {{
                            formData.image.split('/').pop() }}</p>
